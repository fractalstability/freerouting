...
class FreeroutingPlugin(pcbnew.ActionPlugin):
    ...
    def prepare(self):
        ...
        self.route_gnd = config.getboolean("module", "route_gnd")
        self.interactive = config.getboolean("module", "interactive")
        self.threading = config.getboolean("module", "threading")
        ...
        for l in fr:
            ...
            elif (l[:28] == "    (plane GND (polygon F.Cu") and (
                self.route_gnd == True
            ):
                self.bEatNextLine = True
            elif (l[:28] == "    (plane GND (polygon B.Cu") and (
                self.route_gnd == True
            ):
                self.bEatNextLine = True
            else:
        ...
        if self.threading == False:
            self.module_command.extend(["-mt 0"])

        self.module_command.extend(["-de", self.module_input])

        if self.interactive == False:
            self.module_command.extend(["-do", self.module_output])

        # # run freeroute in more interactive mode
...
def get_java_version():
    javaPath = "java"
    try:
        javaInfo = subprocess.check_output(
            javaPath + " -version", shell=True, stderr=subprocess.STDOUT
        )
        javaVersion = (
            re.search(r'"[0-9\._]*"', javaInfo.decode().split("\r")[0])
            .group()
            .replace('"', "")
        )
        return javaVersion
...
class ProcessDialog(wx.Dialog):
    def __init__(self, parent, text):
        ...
        self.bttn = wx.Button(
            self, wx.ID_ANY, "Terminate", wx.DefaultPosition, wx.DefaultSize, 0
        )
